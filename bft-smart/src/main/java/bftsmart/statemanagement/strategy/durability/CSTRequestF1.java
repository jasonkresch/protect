...
/**
 * This class is used to define the roles in the Collaborative State Transfer
 * protocol. The recovering replica uses this class to define which replicas
 * should send the checkpoint, log of operations lower and higher portions
 * 
...
public class CSTRequestF1 extends CSTRequest {
	...
	/**
	 * Define and set the attributes of this CST Request according to the algorithm
	 * described in the durability state transfer paper. In summary, the algorithm
	 * defines that, in a situation with three seeders, the seeder in the middle
	 * (the one that took the checkpoint just after the oldest) must send the
	 * checkpoint. The oldest must send the log of the period between the middle
	 * checkpoint until the checkpoint of the newest replica. The newest replica
	 ...
	 * 
	 * @param others
	 *            the other replicas in BFT-SMaRt
	 * @param cid
	 *            the CID that I am requesting
	 * @param globalCheckpointPeriod
	 *            the global checkpoint period in which all replicas should perform
	 *            the checkpoint
	 * @param me
	 *            my id
	 * @return the position of the replica with the latest checkpoint in the others
	 *         array
	 */
