...
/**
 * Stores the data used to transfer the state to a recovering replica. This
 * class serves the three different seeders defined in the CST optimized version
 * for f=1. In that version, the newest replica to take the checkpoint is
 * expected to send the hash of the checkpoint plus the upper portion of the
 * log. The replica which took the checkpoint before that, i.e., the middle
 * replica is expected to send the checkpoint it has plus hashes of the lower
 * and upper portions of the log. The oldest replica to take the checkpoint must
 * send the lower portion of the log. This object must be passed to the state
 * manager class which will combine the replies from the seeders, validating the
 * values and updating the state in the leecher.
 * 
