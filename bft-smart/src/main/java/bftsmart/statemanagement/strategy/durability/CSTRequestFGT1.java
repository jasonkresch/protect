 ...
 * should send the checkpoint, log of opperations lower and higher portions
 * 
...
public class CSTRequestFGT1 extends CSTRequest {
	...
	/**
	 * number of requests to be processed in the recovering replica before validate
	 * the ckp hash
	 */
	private int nbrHashesBeforeCkp;

	/**
	 * number of messages that should be in the batch if the replica is the one to
	 * send the checkpoint it will correspond to the actual log
	 */
	private int logSize;
	...
	public void defineReplicas(int[] processes, int globalCkpPeriod, int replicaId) {
		...
		// The last checkpoint replica plus all replicas minus one to get the imediate
		// replica before the
		// checkpoint
